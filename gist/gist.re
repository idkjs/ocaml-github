...
let very_pretty_json = s =>
  Json.to_string((Yojson.Safe.from_string(s) :> Yojson.t));
let quite_pretty_json = s =>
  Yojson.Safe.pretty_to_string(Yojson.Safe.from_string(s));
let pretty_json = pretty =>
  if (pretty) {very_pretty_json} else {quite_pretty_json};

exception Auth_token_not_found(string);
exception Gist_file_not_found(string);
  ...
  >>= (
    jar =>
      ...
      >>= (
        ...
        | Some(x) => return(x)
        | None =>
          Lwt.fail(Auth_token_not_found("given id not in cookie jar"))
      )
  );
...
let get_personal_access_token_from_github = (user, pass, token_name) =>
  Passwd.get_if_unset(~prompt="Enter Github password: ", pass)
  >>= (
    pass =>
      M.run(complete_2fa(G.Token.get_all(~user, ~pass)))
      >>= (
        tokens =>
          try(
            switch (token_name) {
            | "" =>
              ...
              return(List.find(a => List.mem(`Gist, a.auth_scopes), tokens))
            | _ =>
              /* find given token */
              return(
                List.find(a => a.auth_app.app_name == token_name, tokens),
              )
            }
          ) {
          | _ => fail(Auth_token_not_found("couldn't find a matching token"))
          }
      )
  );
...
let get_auth = (auth_id, user, pass, token_name) =>
  ...
  | _ =>
    Lwt.fail(
      Auth_token_not_found("must specify either username or jar token id"),
    )
  };

let login = (auth_id, user, pass, token_name, json, pretty) =>
  Lwt_main.run(
    get_auth(auth_id, user, pass, token_name)
    >>= (
      code =>
        if (json) {
          Lwt_io.printf(
...
let list_your_gists = (auth_id, user, pass, token_name, json, pretty) =>
  Lwt_main.run(
    ...
    >>= (
      code => {
        let token = G.Token.of_auth(code);
        M.run(G.Stream.to_list(Gist.all(~token, ())))
        >>= (
          gists =>
            if (json) {
              Lwt_io.printf(
                "%s",
                pretty_json(pretty, Github_j.string_of_gists(gists)),
              );
            } else {
              return(List.iter(describe_gist, gists));
            }
        );
      }
    ),
...
let list_user_gists =
    (auth_id, user, pass, token_name, json, pretty, username) =>
  Lwt_main.run(
    get_auth(auth_id, user, pass, token_name)
    >>= (
      code => {
  ...
  Lwt_main.run(
    ...
    >>= (
      code => {
        ...
        /* get file contents */
        let contents = fname =>
          Lwt_io.(with_file(~mode=input, fname, read))
          >>= (
            new_gist_content =>
              Lwt.return((
                fname,
                {Github_t.new_gist_content: new_gist_content},
              ))
          );

        Lwt_list.map_s(contents, files)
        >>= (
          new_gist_files => {
            let gist = {
            ...
            M.(run(Gist.create(~token, ~gist, ()) >|= G.Response.value))
            >>= (gist => return(describe_gist(gist)));
          }
        );
      }
    ),
...
let string_of_size = x => {
  ...
  if (x < 1024) {
    ...
    sprintf("%i KiB", rnd(x, 1024));
  } else if (x < 1024 * 1024 * 1024) {
    sprintf("%i MiB", rnd(x, 1024 * 1024));
  } else {
    "> GiB!";
  };
};
...
let print_gist_file_info = (name, file) => {
  ...
  Lwt_io.printf(
    "%-40s %-8s %s\n",
    name,
    string_of_size(file.gist_file_size),
    flags,
  );
};
        ...
        >>= (
          gist =>
            if (json) {
              ...
              Lwt_list.iter_s(
                ((name, file)) => print_gist_file_info(name, file),
                gist.gist_files,
              );
            }
        );
...
let gist_file_info =
    (auth_id, user, pass, token_name, json, pretty, gist_id, file) =>
  Lwt_main.run(
    get_auth(auth_id, user, pass, token_name)
    >>= (
      code => {
        ...
        >>= (
          gist =>
            (
              ...
              | _ => Lwt.fail(Gist_file_not_found(file))
              }
            )
            >>= (
              file_data =>
                if (json) {
                  Lwt_io.printf(
                    ...
                    pretty_json(
                      pretty,
                      Github_j.string_of_gist_file(file_data),
                    ),
                  );
                } else {
                  print_gist_file_info(file, file_data)
                  >>= (
                    () =>
...
let json =
  Arg.(
    value & flag & info(["json"], ~docv="JSON", ~doc="Show JSON responses.")
  );
...
let pretty =
  Arg.(
    value
    & flag
    & info(["pretty"], ~docv="PRETTY", ~doc="Pretty print JSON responses.")
  );
...
let file_pos =
  Arg.(
    ...
    & info([], ~docv="FILENAME", ~doc="File name.")
  );

let list_your_gists = (
  Term.(
    pure(list_your_gists) $ auth_id $ user $ pass $ token_name $ json $ pretty
  ),
  ...
  let public =
    Arg.(
      ...
      & info(
          ["public"],
          ~docv="PUBLIC GIST",
          ~doc="Create a public gist (default is secret)",
        )
    );
  let descr =
    Arg.(
...
let login = (
  Term.(pure(login) $ auth_id $ user $ pass $ token_name $ json $ pretty),
...
let default_cmd = {
  let doc = "manipulate Github GIST files from the command line";
  (
    Term.(ret(pure(`Help((`Pager, None))))),
    {
      let man = [
        `S("DESCRIPTION"),
        `P(
          "Read, write and otherwise manipulate Github GIST files from the command line.  Github authentication is handled with tokens created with the $(b,git-jar) command line tool.",
        ),
        `S("AUTHORIZATION OPTIONS"),
        `P(
          "An authorization token is required to access the Github API.  You can generate and store a token using the $(b,git-jar) tool and retrieve it with $(b,--auth-id).  Alternatively you can supply a username and password to retrieve a token from Github (specified with $(b,--token-id) or otherwise found automatically).",
        ),
...
let () =
  switch (Term.eval_choice(default_cmd, cmds)) {
  | `Error(_) => exit(1)
  | _ => exit(0)
  };
