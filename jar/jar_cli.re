      ...
      >>= (
        jar =>
          ...
          >|= (
            ...
            | None => {
                eprintf(
                  "Missing cookie: use git-jar to create cookie `%s`.\n%!",
                  cookie,
                );
                exit(1);
              }
...
let repos = (~doc_append="", ()) => {
  let doc = "Repositories in user/repo format" ++ doc_append;
  Arg.(non_empty & pos_all(string, []) & info([], ~docv="REPOS", ~doc));
};

let cookie = (~doc_append="", ()) => {
  let doc = "Authentication cookie" ++ doc_append;
  let env = Arg.env_var("GH_COOKIE");
  map(
    auth,
    Arg.(
      value & opt(string, "infra") & info(~env, ["c"], ~docv="COOKIE", ~doc)
    ),
  );
};
